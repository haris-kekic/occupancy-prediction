...
from model_rnn_lstm import RNN_LSTM
...
rnn_artifacts_path = 'experiments/rnn'
...
def generate_experiment_hyperparams(num_of_files=4):
    ...
    # Minuten: 6std, 12std 3Tage
    TIMESTEPS = [30, 96, 288]

    # Minuten: 6Std, 24Std, 3Tage
    FORECASTS = [30, 96, 288]
            ...
            for feat_comb in FEAT_COMB:
                ...
                for time_step in TIMESTEPS:
                    ...
                    for forecast in FORECASTS:
                        ...
                        for stack_unit in STACKS_UNITS:
                            ...
                            for reg_comb in regular_dropout_comb:
                                ...
                                df_experiment_row = Series({'Dataset': ';'.join(ds), 
                                                            ...
                                                            'StacksAndUnits': ';'.join(stack_unit), 
    ...
    for i in range(0, df_experiments.shape[0], split_step):
        ...
        df_exp_split[['Dataset', 
                    ...
                    'Splits',
                    'Epochs',
                    'Done']].to_csv(f'{rnn_artifacts_path}/experiments_part{en}.csv')
        en = en + 1
...
def run_experiment(file_path):
    ...
    for exp_num, row in df_experiments[df_experiments['Done'] == False].iterrows():
        ...
        stack_units = [int(i) for i in str(row.StacksAndUnits).split(';')]
        ...
        df_dataset = dp.read_datasets(datasets, algo) 
        df_dataset = df_dataset[feat_labs]
        # Erstelle das RNN LSTM Modell aufgrund der Parameter aus der Experiment-Datei
        model = RNN_LSTM((stack_units, use_reg, use_drop))
        # Starte das Training und Evaluierung mit dem Datensatz auf dem erstellten Modell
        validator = ModelValidator('LSTM', model, df_dataset)
        ...
        csv_results_path = f'{rnn_artifacts_path}/results_{exp_num}'
        ...
        visualizer.visualize_model_scores(df_aggr_score, best_val_loss, exp_num, rnn_artifacts_path) # Hier auch noch die Accuracy und Validierungsdiagramme
...
def run_experiments_async():
    ...
    '''
    experiment_files = glob.glob(f"{rnn_artifacts_path}/*.csv")
    processes = list()
...
def compose_all_results():   
    ''' Kann erst nach dem Experiment ausgeführt werden um die besten Ergebnisse
        aus den Ordnern zusammenzufassen und in eine Datei zu speichern.
        Separat für Loss und Accuracy werden zwei Dateien mit den besten Ergebnissen zusammengestellt.
    ''' 
    result_folders = glob.glob(f"{rnn_artifacts_path}/results_*/")
    df_best_losses = DataFrame(columns=["Id", "ResultId", "Dataset", "Algo", "FeaturesLabels", "TimeSteps", "ForecastSteps", "StackUnits", "Epoch", "EpochSmoothValAcc", "EpochSmoothValLoss", "EpochTrainAcc", "EpochTrainLoss", "EpochValAcc", "EpochValLoss"], index=None)
    df_best_accs = DataFrame(columns=["Id", "ResultId", "Dataset", "Algo", "FeaturesLabels", "TimeSteps", "ForecastSteps", "StackUnits", "Epoch", "EpochSmoothValAcc", "EpochSmoothValLoss", "EpochTrainAcc", "EpochTrainLoss", "EpochValAcc", "EpochValLoss"], index=None)
    ...
    for result_folder in result_folders:
        ...
        stack_units = df_parameter.iloc[5, 1]

        # Hinzufügen von besten Losses zum Hauptframe
        res_ids = np.full( shape=df_score_best_loss.shape[0], fill_value=res_id, dtype=np.int)
        ...
        stack_unitss = np.full( shape=df_score_best_loss.shape[0], fill_value=stack_units)
        df_score_best_loss["ResultId"] = res_ids
        ...
        df_score_best_loss["StackUnits"] = stack_unitss
        df_best_losses = df_best_losses.append(df_score_best_loss)

        # Hinzufügen von besten Accuracies zum Hauptframe
        res_ids = np.full( shape=df_score_best_acc.shape[0], fill_value=res_id, dtype=np.int)
        ...
        stack_unitss = np.full( shape=df_score_best_acc.shape[0], fill_value=stack_units)
        df_score_best_acc["ResultId"] = res_ids
        ...
        df_score_best_acc["StackUnits"] = stack_unitss
    ...
    df_best_losses.to_csv(f"{rnn_artifacts_path}/all_results/score_best_losses.csv")
    df_best_accs.to_csv(f"{rnn_artifacts_path}/all_results/score_best_accs.csv")
...
def get_all_results():
    ...
    ''' 
    df_best_losses = pd.read_csv(f"{rnn_artifacts_path}/all_results/score_best_losses.csv", index_col="Id")
    df_best_accs = pd.read_csv(f"{rnn_artifacts_path}/all_results/score_best_accs.csv", index_col="Id")
...
def arrange_best_scores(title, dataset, algo='std'):
    ...
    df_arranged_losses.to_csv(f"{rnn_artifacts_path}/all_results/score_{title}_{algo}_best_losses.csv")
    df_arranged_accs.to_csv(f"{rnn_artifacts_path}/all_results/score_{title}_{algo}_best_acc.csv")
...
def evaluate_model(id, dataset_eval, epochs):
    ...
    '''
    experiment_files = glob.glob(f"{rnn_artifacts_path}/experiments*.csv")
    ...
    for file_path in experiment_files:
        ...
        model = RNN_LSTM((stack_units, use_reg, use_drop))
        validator = ModelValidator('RNN LSTM', model, df_datasets, df_datasets_eval)
...
def evaluate_models(title, *eval_items):
    ...
    df_eval_results.to_csv(f"{rnn_artifacts_path}/all_results/score_{title}_eval.csv")
